...
async def _(select: KMLSelect | list[KMLSelect]) -> None:
    ...
    for s in select_list:
        target = next(filter(lambda x: x.id == str(s.id), tracker.data), None)
        if target is None:
            continue
        if target in tracker.sync and not s.checked:
            tracker.sync.remove(target)
            target.force_idle()
        elif target not in tracker.sync and s.checked:
            tracker.sync.append(target)
